...
import { evaluateHkdf }   from './pseudorandom-number-generators/hkdf.js';
import { createShake256 } from './pseudorandom-number-generators/shake.ts';

// -----------------------------------------------------------------------------
// constant values and static methods
...
function getCurveParams(curve) {
    ...
    switch (sjcl.ecc.curveName(curve)) {
        ...
        default:
            throw new Error(
                '[privacy-pass]: Incompatible curve chosen for H2C: ' + sjcl.ecc.curveName(curve),
            );
    }
    return curveParams;
}
...
export function getBase64FromBytes(bytes) {
    const bits = sjcl.codec.bytes.toBits(bytes);
    const encoded = sjcl.codec.base64.fromBits(bits);
    return encoded;
}

export function getBase64FromString(str) {
    const bits = sjcl.codec.utf8String.toBits(str);
    const encoded = sjcl.codec.base64.fromBits(bits);
    return encoded;
}
...
export class VOPRF {
    ...
    constructor(h2cParams) {
        ...
        switch (curveStr) {
            case 'p256':
                ...
                this.CURVE_H2C_HASH = sjcl.hash.sha256;
    ...
    sec1DecodeFromBytes(sec1Bytes) {
        ...
        switch (sec1Bytes[0]) {
            ...
            case 0x03:
                ...
                break;
            case 0x04:
                P = this.CURVE.fromBits(sjcl.codec.bytes.toBits(sec1Bytes.slice(1)));
                break;
            default:
                throw new Error(
     ...
     */
    blindPoint(P) {
        const bF = sjcl.bn.random(this.CURVE.r, 10);
     ...
     */
    unblindPoint(b, Q) {
        const binv = b.inverseMod(this.CURVE.r);
     ...
     */
    decompressPoint(bytes) {
        const yTag = bytes[0];
        const expLength = this.CURVE.r.bitLength() / 8 + 1; // bitLength rounds up
        if (yTag != 2 && yTag != 3) {
        ...
        const x = this.CURVE.field.fromBits(sjcl.codec.bytes.toBits(xBytes)).normalize();
        const sign = yTag & 1;

        // y^2 = x^3 - 3x + b (mod p)
        let rh = x.power(3);
        const threeTimesX = x.mul(this.CURVE.a);
        rh = rh.add(threeTimesX).add(this.CURVE.b).mod(this.CURVE.field.modulus); // mod() normalizes

        // modsqrt(z) for p = 3 mod 4 is z^(p+1/4)
        const sqrt = this.CURVE.field.modulus.add(1).normalize().halveM().halveM();
        let y = new this.CURVE.field(rh.powermod(sqrt, this.CURVE.field.modulus));
        ...
        const point = new sjcl.ecc.point(this.CURVE, x, y);
        if (!point.isValid()) {
     ...
     */
    parsePublicKeyfromPEM(pemPublicKey) {
        try {
            let bytes = PEM.parseBlock(pemPublicKey);
            let json = ASN1.parse(bytes.der);
            let xy = json.children[1].value;
            const point = this.sec1DecodeFromBytes(xy);
            return new sjcl.ecc.ecdsa.publicKey(this.CURVE, point);
        } catch (e) {
     ...
     */
    verifyConfiguration(publicKey, config, signature) {
        const sig = parseSignaturefromPEM(signature);
        const msg = JSON.stringify(config);
        const pk = this.parsePublicKeyfromPEM(publicKey);
     ...
     * @return {boolean}
     */
    verifyProof(proofObj, tokens, signatures, commitments, prngName) {
        ...
        const pointG = this.sec1DecodeFromBase64(commitments.G);
        const pointH = this.sec1DecodeFromBase64(commitments.H);
        ...
        const composites = this.recomputeComposites(tokens, signatures, pointG, pointH, prngName);
        ...
        const h = new this.CURVE_H2C_HASH(); // use the h2c hash for convenience
     ...
     */
    recomputeComposites(tokens, signatures, pointG, pointH, prngName) {
        const seed = this.computeSeed(tokens, signatures, pointG, pointH);
        let cM = new sjcl.ecc.pointJac(this.CURVE); // can only add points in jacobian representation
        let cZ = new sjcl.ecc.pointJac(this.CURVE);
        ...
        for (let i = 0; i < tokens.length; i++) {
            iter++;
            const ci = this.computePRNGScalar(prng, seed, new sjcl.bn(iter).toBits());
     ...
     */
    computePRNGScalar(prng, seed, salt) {
        const bitLen = this.CURVE.r.bitLength();
     ...
     */
    computeSeed(chkM, chkZ, pointG, pointH) {
        const compressed = chkZ.compressed;
        const h = new this.CURVE_H2C_HASH(); // we use the h2c hash for convenience
    ...
    deriveKey(N, token) {
        ...
        const hash = this.getActiveECSettings().hash;
    ...
    getCurvePoints(signatures) {
        ...
        sigBytes.forEach(function (buf) {
            const usablePoint = self.sec1DecodeFromBytes(buf);
            if (usablePoint == null) {
    ...
    hashAndInc(seed, hash, label) {
        ...
        for (i = 0; i < 20; i++) {
            ...
            const point = this.sec1DecodeFromBytes([2].concat(bytes));
            if (point !== null) {
                return point;
            }
